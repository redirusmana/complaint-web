...
import { colorSet } from '../../configs/color';
...
const TableRowWrapper = ({ record, onClickRow, children }) => {
  ...
  if (typeof onClickRow === 'object' && typeof onClickRow.pathname !== 'undefined') {
    const anchor = getUrlFromHref(onClickRow.pathname);
    return (
      <Link to={`${anchor.pathname}/${record[onClickRow.recordKey]}`} className="chart-table-item">
        {children}
      </Link>
    );
  }
...
const InnerTableMini = ({ dataSource, formats, tableName, showNumber, onClickRow }) => {
  if (dataSource.length === 0) {
    return (
      <TableRowWrapper>
        {formats.map(format => {
          if (format.className === 'color') {
            return <span className="chart-table-color" key={`table-mini-cell-${tableName}-empty-${format.key}`} />;
          }
  ...
  const result = dataSource.map((datum, index) => (
    <TableRowWrapper onClickRow={onClickRow} record={datum} key={`table-mini-row-${tableName}-${index}`}>
      ...
      {formats.map(format => {
        ...
        if (format.className === 'desc') {
          const tooltipKey = typeof format.tooltipKey !== 'undefined' ? format.tooltipKey : format.key;
          return (
            <Tooltip
              key={`table-mini-cell-${tableName}-${index}-${format.key}`}
...
const TableMini = ({
  ...
  <div className={`chart-table-mini ${className}`}>
    {showTopHeader && (
      <div className="chart-table-item chart-table-header">
        ...
        {topFormat.map((format, indexFormat) => (
          <span className={`chart-table-${format.className}`} key={`${tableName}-table-top-header-${indexFormat}`}>
            {format.label}
          </span>
        ))}
      </div>
    ...
    {showHeader && (
      <div className="chart-table-item chart-table-header">
        {showNumber && <span className="chart-table-percent">{labelNumber}</span>}
        {formats.map(format => (
          <span className={`chart-table-${format.className}`} key={`${tableName}-table-header-${format.key}`}>
            {format.label}
          </span>
        ))}
      </div>
...
TableMini.propTypes = {
  ...
  onClickRow: PropTypes.oneOfType([PropTypes.bool, PropTypes.func, PropTypes.shape({})])
};
